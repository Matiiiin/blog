...
#
    def test_user_post_list_view_template_used(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, "account/dashboard/user_posts.html")
...
class TestUserPostCreateView(TestCase):
    ...
    def test_user_post_create_view_template_used(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, "account/dashboard/user_post_create.html")

    def test_user_post_create_view_form_valid(self):
        image_file_1 = SimpleUploadedFile("test_image_1.jpg", b"file_content", content_type="image/jpeg")
        image_file_2 = SimpleUploadedFile("test_image_2.jpg", b"file_content", content_type="image/jpeg")
        image_file_3 = SimpleUploadedFile("test_image_3.jpg", b"file_content", content_type="image/jpeg")
...
class TestUserPostUpdateView(TestCase):
    def setUp(self):
        self.client = Client()
        self.url = reverse("account:user-post-update", kwargs={"slug": "test-post"})
        self.client.login(username="testuser", password="password")
    ...
    def test_user_post_update_view_template_used(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, "account/dashboard/user_post_update.html")
    ...
    def test_user_post_update_view_form_valid(self):
        ...
        self.assertTrue(Post.objects.filter(title="Updated Test Post").exists())
...
class TestUserPostDeleteView(TestCase):
    def setUp(self):
        self.client = Client()
        self.url = reverse("account:user-post-delete", kwargs={"slug": "test-post"})
        self.client.login(username="testuser", password="password")
    ...
    def test_user_post_delete_view_template_used(self):
        response = self.client.get(self.url)
        self.assertTemplateUsed(response, "account/dashboard/user_post_delete.html")

...
class TestCommentModelViewSet:
    ...
    def test_update_comment(self, api_client, user, comment, post):
        api_client.force_authenticate(user=user)
        url = reverse("blog:api-v1:comment-detail", args=[comment.id])
        data = {"content": "Updated Comment", "post": post.slug}
    ...
    def test_create_comment_non_verified(
    ...
    def test_delete_comment_non_owner(
        self, api_client, user, comment
    ):
        non_owner_user = User.objects.create_user(
            username="non_owner",
...
class TestCommentReplyModelViewSet:
    ...
    def test_retrieve_comment_reply(
        self, api_client, user, comment_reply
    ):
        api_client.force_authenticate(user=user)
        url = reverse(
            "blog:api-v1:comment-reply-detail",
            args=[comment_reply.id],
        )
        response = api_client.get(url)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_comment_reply(
        ...
        data = {
            "content": "Updated Comment Reply",
            "comment": comment_reply.comment.id,
        }
        response = api_client.put(url, data)
        assert response.status_code == status.HTTP_200_OK

    def test_partial_update_comment_reply(
        self, api_client, user, comment_reply
    ):
        api_client.force_authenticate(user=user)
        url = reverse(
    ...
    def test_create_comment_reply_non_verified(
        self, api_client, user, comment
    ):
        user.is_verified = False
        user.save()
    ...
    def test_update_comment_reply_non_owner(
        self, api_client, user, comment_reply
    ):
        non_owner_user = User.objects.create_user(
            username="non_owner",
    ...
    def test_delete_comment_reply_non_owner(
        self, api_client, user, comment_reply
    ):
        non_owner_user = User.objects.create_user(
            username="non_owner",

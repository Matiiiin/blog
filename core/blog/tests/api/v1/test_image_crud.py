...
class TestImageGenericViewSet:
    ...
    def test_anonymous_user_image_retrieve_update_destroy(
        self, api_client, image
    ):
    ...
    def test_normal_user_cant_update_image(
        self, api_client, user, image, image_file
    ):
        api_client.force_authenticate(user=user)
        url = reverse("blog:api-v1:image-detail", args=[image.id])
        data = {"image": image_file}
    ...
    def test_admin_user_can_update_image(
        self, api_client, user, image, image_file
    ):
        user.is_staff = True
        user.save()
    ...
    def test_normal_user_cant_delete_image(
    ...
    def test_admin_user_can_delete_image(
        self, api_client, user, image
    ):
        user.is_staff = True
        user.save()
